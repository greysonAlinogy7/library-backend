...
import com.book.library.mappers.BookMapper;
...
import com.book.library.service.impl.IBookService;
...
public class BookService implements IBookService {
    ...
    public List<BookDTO> createBookBulk(List<BookDTO> bookDTOs) throws BookException {
        List<BookDTO> createBooks = new ArrayList<>();
        for (BookDTO bookDTO : bookDTOs){
          BookDTO book =  createBook(bookDTO);
          createBooks.add(book);
        }
        return createBooks;
    }
    ...
    public BookDTO updateBook(Long bookId, BookDTO bookDTO) throws BookException {
        ...
        bookMapper.updateEntity(bookDTO, existingBook);
    ...
    @Override
    public PageResponse<BookDTO> searchBookWithFilter(BookSearchRequest searchRequest) {
    ...
    private PageResponse<BookDTO> convertToPageResponse(Page<Book> books){
        List<BookDTO> bookDTOS = books.getContent()
                .stream().map(bookMapper::bookDTO).collect(Collectors.toList());
